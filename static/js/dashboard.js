  ...
  for (let i = 0; i < lessonsData.length; i++) {
    ...
    lessons.append(`
            <div class="lesson scale-up" id="${lesson}">
                <a href="${lessonInfo.url}"><img src="${
      lessonImages[lesson]
    }" alt="${lessonInfo.title}"></a>
            </div>
        `);
    ...
    if (modulesData.hasOwnProperty(moduleName)) {
      ...
      if (foundMarkedModule) {
        ...
        $newModule.click(evt => {
          displayLessons(
            module.games,
            MODULE_NAMES.indexOf(moduleName) <
              MODULE_NAMES.indexOf(profile.preferredProgress[0])
          );
...
function getModuleHtml(module) {
  if (moduleImages[module.name]) {
    return `<div class="module scale-up" id="${module.name}">
                    <img class="module-img" src="${
                      moduleImages[module.name]
                    }" alt="${module.name}">
                </div>`;
  } else {
    return `<div class="module scale-up">
                    <h2 id="${module.name}" class="module-name"><a href="#">${
      module.name
    }</a></h2>
                    <p>${module.desc}</p>
