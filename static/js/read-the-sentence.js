...
function setupRecognition() {
  ...
  recognition.onresult = function(event) {
    ...
    if (
      interimTranscriptText.trim() === "" &&
      sentence
        .toLowerCase()
        .trim()
        .replace(".", "")
        .replace("!", "")
        .replace("?", "") === finalTranscriptText.trim()
    ) {
      recognition.stop();
...
function checkAnswer() {
  ...
  if (finalTranscriptText.trim() === "") {
    $("#message-text").text(
      "Huh? We didn't here anything! Press the button and try saying the sentence again!"
    );
    $("#message").animate(
      {
        opacity: 1
    ...
    $("#message-text").text(
      "You've tried a few times already, how about coming back to this one later?"
    );
    $("#message").animate(
      {
        opacity: 1
    ...
    $("#message-text").text(
      "You're pronouncing the words correctly, but try saying the whole sentence in the right order!"
    );
    $("#message").animate(
      {
        opacity: 1
